...
let componentFlags : NSCalendar.Unit = [.NSYearCalendarUnit, .NSMonthCalendarUnit, .NSDayCalendarUnit, .NSWeekCalendarUnit, .NSHourCalendarUnit, .NSMinuteCalendarUnit, .NSSecondCalendarUnit, .NSWeekdayCalendarUnit, .NSWeekdayOrdinalCalendarUnit];
extension NSDate{
    ...
    var nearestHour:NSInteger{
        ...
        let newDate = NSDate(timeIntervalSinceReferenceDate:aTimeInterval);
        let components = NSDate.currentCalendar().components(NSCalendar.Unit.NSHourCalendarUnit, from: newDate as Date);
